...
try{ 
  ...
  insert into tb_usuario(
    nome,sobrenome,endereco,telefone,login,senha,tb_tipo_id
  ) values (
 ...
 //rowCount() é igual a 1 (quantidade de linhas que forma inseridas)
    if($preparacao->rowCount()==1){
        ...
        header(
            'location:../../paginas/cad-usuario/usuario.php?status=201' );
        ...
        header(
            'location:../../paginas/cad-usuario/usuario.php?status=500');
            //Morre a execução para evitar lacunas de segurança
            die();
    }
